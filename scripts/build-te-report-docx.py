...
class Report:
    ...
    def ch1(self):
        ...
        self.p(
            ...
            "เพื่อเพิ่มรัศมีวงเลี้ยว แก้ไขปัญหาจราจรติดขัดบริเวณถนนแจ้งวัฒนะเลี้ยวซ้ายเข้าถนนกำแพงเพชร 6 ด้านทิศเหนือ"
        )
        self.p(
            "รายงานวิเคราะห์ฉบับก่อนใช้ทิศ NB เลี้ยวซ้าย และมีค่า PCU บางช่วงผิดปกติ "
        ...
        self.p(
            ...
            "ในระดับ OFFSET ไม่จำลองจุลภาคทั้งสายแจ้งวัฒนะ และไม่ประเมินอุบัติเหตุย้อนหลังเพราะไม่มีแฟ้มอุบัติเหตุประกอบการศึกษา"
        )
    ...
    def ch10(self):
        ...
        self.h("10.1  สรุปผลหลัก", 2)
        self.p(
            f"ปริมาณช่วงเร่งด่วน — เช้า 07:00–09:00 รวม {th(am)} คัน เย็น 16:00–18:00 รวม {th(pm)} คัน "
            f"พีคจริงของทิศนี้คือ 12:00–13:00 ที่ {th(self.demand)} คัน/ชม. จากแผ่น Report"
        )
        self.p(
            f"ใช้เส้นทางนี้ทั้งวัน — {th(self.s['eb5_12h'])} คัน ใน 12 ชั่วโมง "
            f"({th(self.s['eb5_12h_pcu'], 1)} PCU) คิดเป็น {self.s['eb5_12h'] / self.s['ix_12h'] * 100:.1f}% ของรถทั้งแยก"
        )
        self.p(
            f"ผลของผายปาก — ความจุเหลือเฟือ (v/c {free_x:.2f} LOS {los_un(free_d)}) "
